//! New-API port of `banner_snail.addVectorSnail`.
//!
//! Builds the vector snail as a sequence of (path, paint, transform)
//! shapes using `pathToCurves` / `strokeToCurves` and direct Atlas
//! entries. The legacy `PathPictureBuilder.addPath` "fill + inside
//! stroke" composite mode isn't reproduced exactly — fills and strokes
//! are emitted as separate shapes drawn in order (stroke after fill).
