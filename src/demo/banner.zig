...
const BannerBuilder = struct {
    ...
    fn emitShapedRunPaint(
        ...
        for (shaped.glyphs) |g| {
            ...
            const curves = try font_ref.extractCurves(self.allocator, &self.glyph_caches[fid], g.glyph_id);
        ...
        for (shaped.glyphs) |g| {
            ...
            if (iter.count() > 0) {
                while (iter.next()) |layer| {
                    ...
                    const curves = try font_ref.extractCurves(self.allocator, cache, layer.glyph_id);
                    try self.text_curves_owned.append(self.allocator, curves);
                    try self.text_entries.append(self.allocator, .{
                        .key = layer_key,
                        .curves = self.text_curves_owned.items[self.text_curves_owned.items.len - 1],
            ...
            if (containsKey(self.text_entries.items, key)) continue;
            const curves = try font_ref.extractCurves(self.allocator, cache, g.glyph_id);
            try self.text_curves_owned.append(self.allocator, curves);
            try self.text_entries.append(self.allocator, .{
                .key = key,
                .curves = self.text_curves_owned.items[self.text_curves_owned.items.len - 1],
