...
pub fn GlDeviceAtlasFor(comptime variant: Variant) type {
    ...
    return struct {
        ...
        fn texSubImage3D(self: *Self, tex: gl.GLuint, unit: gl.GLenum, layer: u32, col_base: u32, row_base: u32, width: u32, height: u32, format: gl.GLenum, ty: gl.GLenum, ptr: ?*const anyopaque) void {
            _ = self;
            if (comptime variant.supportsDsa()) {
                gl.glTextureSubImage3D(tex, 0, @intCast(col_base), @intCast(row_base), @intCast(layer), @intCast(width), @intCast(height), 1, format, ty, ptr);
            } else {
                gl.glActiveTexture(@intCast(@as(i64, @intCast(gl.GL_TEXTURE0)) + @as(i64, @intCast(unit))));
                gl.glBindTexture(gl.GL_TEXTURE_2D_ARRAY, tex);
                gl.glTexSubImage3D(gl.GL_TEXTURE_2D_ARRAY, 0, @intCast(col_base), @intCast(row_base), @intCast(layer), @intCast(width), @intCast(height), 1, format, ty, ptr);
            }
        }
