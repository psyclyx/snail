...
pub const VulkanDeviceAtlas = struct {
    ...
    fn appendRegions(scratch: std.mem.Allocator, batch: *UploadBatch, regions: []const upload_plan.Region) UploadError!void {
        for (regions) |r| switch (r.target) {
            .curve => try batch.curve_ops.append(scratch, .{ .src = r.src, .layer = r.layer, .width = r.width, .height = r.height }),
            .band => try batch.band_ops.append(scratch, .{ .src = r.src, .layer = r.layer, .width = r.width, .height = r.height }),
