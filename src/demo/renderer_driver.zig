...
pub fn warnIfDebugCpu(kind: Kind) void {
    if (kind == .cpu and builtin.mode == .Debug) {
        std.debug.print(
    ...
    pub fn init(allocator: std.mem.Allocator, window: *wayland.Window, selected: Kind) !Driver {
        return switch (selected) {
            ...
            .cpu => if (comptime build_options.enable_cpu)
                .{ .cpu = try CpuDriver.init(allocator, window) }
            else
                unreachable,
        };
    }
...
const CpuDriver = if (build_options.enable_cpu) struct {
    ...
    fn init(allocator: std.mem.Allocator, window: *wayland.Window) !CpuDriver {
        ...
        const pool = try allocator.create(snail.ThreadPool);
        errdefer allocator.destroy(pool);
        try pool.init(allocator, .{});
        errdefer pool.deinit();

        renderer_state.setThreadPool(pool);
        return .{
            .allocator = allocator,
