//! Shared content builder for the new-API screenshot demos.
//!
//! Each backend (`screenshot_new.zig` for CPU, `_gl`, `_gles30`, `_vulkan`)
...
pub const Content = struct {
    ...
    text_atlas_new: snail.Atlas,
    paths_picture: snail.Picture,
    text_pic: snail.Picture,

    pub fn deinit(self: *Content) void {
        self.text_pic.deinit();
        self.paths_picture.deinit();
        self.text_atlas_new.deinit();
...
pub fn build(allocator: Allocator, width: u32, height: u32) !Content {
    // -- Shape text via the existing TextAtlas.
    ...
    errdefer text_atlas_new.deinit();
    ...
    errdefer text_pic.deinit();

    return .{
        .allocator = allocator,
        .pool = pool,
        .paths_atlas = paths_atlas,
        .text_atlas_new = text_atlas_new,
