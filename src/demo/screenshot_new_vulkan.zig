//! Vulkan counterpart to `screenshot_new.zig`. Renders the same
//! Picture content through the new-API Vulkan upload + draw path
//! (Phase 5c) and writes `zig-out/demo-screenshot-new-vulkan.tga`.
...
pub fn main() !void {
    ...
    var cache = try snail.VulkanPreparedPages.init(allocator, pool, vk_renderer.state.newApiPipelineShape());
    defer cache.deinit();
