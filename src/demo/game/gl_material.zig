...
//! lighting it over an opaque panel. `zig build gen-shaders` checks the
//! translated GL dialects in under src/demo/game/generated/; this file embeds
...
pub fn GlMaterial(comptime variant: Variant) type {
    return struct {
        ...
        // Generated by `zig build gen-shaders` from game_material.slang
        // (snail's GL 3.3 and 4.4 paths share the #version 330 dialect; DSA
        // is an API concern, not a shader-dialect one).
        const vertex_src: [:0]const u8 = switch (variant) {
            .gl33, .gl44 => @embedFile("generated/glsl330/game_material.vert.glsl"),
            .gles30 => @embedFile("generated/gles300/game_material.vert.glsl"),
        };
        const fragment_src: [:0]const u8 = switch (variant) {
            .gl33, .gl44 => @embedFile("generated/glsl330/game_material.frag.glsl"),
            .gles30 => @embedFile("generated/gles300/game_material.frag.glsl"),
        };
