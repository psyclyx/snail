...
const pipeline = if (build_options.enable_opengles) @import("../backend/gles/state.zig") else struct {
...
const Config = if (build_options.enable_opengles) struct {
    pub const Backend = pipeline.GlesTextState;
    pub const Prepared = pipeline.PreparedResources;
    pub const backend_kind = interface.BackendKind.gles;
    pub const uses_resource_cache = true;

    pub fn prepared(prepared_resources: *const PreparedResources) ?*const Prepared {
        return prepared_resources.resident.gles orelse null;
    }
    ...
    pub fn uploadResources(self: *Backend, allocators: UploadAllocators, prepared_resources: *PreparedResources, batch: ResourceUploadBatch) !void {
        const gles_prepared = self.resourceCache(allocators.persistent);
        if (batch.atlases.len > 0 or batch.layer_infos.len > 0) try gles_prepared.uploadAtlasesAndLayerInfoWithCapacityModes(
            allocators.scratch,
            batch.atlases,
        ...
        if (batch.images.len > 0) try gles_prepared.uploadImages(allocators.scratch, batch.images, batch.image_views);
        prepared_resources.resident.gles = gles_prepared;
        prepared_resources.resident.generation = gles_prepared.generation;
    }

    pub fn coverageBackend(self: *Backend, prepared_resources: *const PreparedResources) ?CoverageBackend {
        if (prepared_resources.resident.gles) |gles_resources| {
            return .{ .gles = .{ .gles = self, .gles_resources = gles_resources, .prepared = prepared_resources } };
        }
        return null;
    }
    ...
    pub fn drawPass(renderer: *ErasedRenderer, prepared_resources: *const PreparedResources, records: DrawRecords, pass: DrawPass) anyerror!void {
        switch (pass.resolve) {
            ...
            .linear => |resolve| {
                const gles_state: *Backend = @ptrCast(@alignCast(renderer.ptr));
                const restore = try gles_state.beginLinearResolve(pass.state.surface, resolve);
                defer gles_state.endLinearResolve(restore);
...
pub const vtable = if (build_options.enable_opengles) common.vtable(Config) else interface.disabledVTable(.gles);

/// Typed handle for the GLES backend.
///
/// `Renderer` owns the GLES state; the upload / draw methods are thin shims over
/// the erased renderer for callers that want to stay strongly typed.
pub const Renderer = if (build_options.enable_opengles) struct {
    const Self = @This();
    allocator: std.mem.Allocator,
    state: *pipeline.GlesTextState,

    pub fn init(allocator: std.mem.Allocator) !Self {
        const text = try allocator.create(pipeline.GlesTextState);
        text.* = .{};
    ...
    pub fn coverageBackend(self: *Self, prepared_resources: *const PreparedResources) ?CoverageBackend {
        if (prepared_resources.resident.gles) |gles_resources| {
            return .{ .gles = .{ .gles = self.state, .gles_resources = gles_resources, .prepared = prepared_resources } };
        }
        return null;
    }
