...
fn PreparedResourcesFor(comptime backend: Backend) type {
    return struct {
        const PreparedResources = @This();
        ...
        fn appendTexturePages(self: *PreparedResources, scratch: std.mem.Allocator, atlases: []const *const CurveAtlas) !bool {
            ...
            if (comptime backend == .gl33) {
                self.uploadTexturePagesGl33WithStarts(atlases, start_pages[0..atlases.len]);
            } else {
                self.uploadTexturePagesGl44WithStarts(atlases, start_pages[0..atlases.len]);
            }
        ...
        fn createAtlasTextureBank(self: *PreparedResources, plan: AtlasAppendPlan) AtlasTextureBank {
            ...
            self.next_atlas_bank_id +%= 1;
            if (comptime backend == .gl33) {
                self.createAtlasTextureBankGl33(&bank, plan);
            } else {
                self.createAtlasTextureBankGl44(&bank, plan);
            }
            return bank;
        }
        ...
        fn uploadAllPages(self: *PreparedResources, atlases: []const *const CurveAtlas) void {
            if (comptime backend == .gl33) {
                self.uploadTexturePagesGl33WithStarts(atlases, null);
            } else {
                self.uploadTexturePagesGl44WithStarts(atlases, null);
            }
        }
...
pub const Gl33PreparedResources = PreparedResourcesFor(.gl33);
pub const Gl44PreparedResources = PreparedResourcesFor(.gl44);

test "prepared resource retirement releases unreferenced GL banks" {
    const allocator = std.testing.allocator;
    var cache = Gl33PreparedResources{ .allocator = allocator, .active_atlas_bank_id = 99 };
...
test "stale GL prepared release does not touch active refs from newer generation" {
    var cache = Gl33PreparedResources{
