    ...
    switch (format) {
        ...
        2 => {
            ...
            while (lo < hi) {
                ...
                const start = try readU16(data, rec);
                const end = try readU16(data, rec + 2);
                if (glyph_id < start) hi = mid else if (glyph_id > end) lo = mid + 1 else return try readU16(data, rec + 4);
            }
            return 0;
        },
        else => return 0,
...
fn readValueRecord(data: []const u8, offset: usize, format: u16) !ValueRecord {
    ...
    var off = offset;
    if (format & 0x0001 != 0) {
        rec.x_placement = try readI16(data, off);
        off += 2;
    }
    if (format & 0x0002 != 0) {
        rec.y_placement = try readI16(data, off);
        off += 2;
    }
    if (format & 0x0004 != 0) {
        rec.x_advance = try readI16(data, off);
        off += 2;
    }
    if (format & 0x0008 != 0) {
        rec.y_advance = try readI16(data, off);
        off += 2;
    }
