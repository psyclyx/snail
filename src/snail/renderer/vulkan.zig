...
const AtlasSlot = upload_common.AtlasSlot(CurveAtlas, AtlasPage);
const ImageSlot = upload_common.ImageSlot(snail_mod.Image);
const AtlasPageUpload = struct {
...
const ResourceBank = struct {
    ...
    fn deinit(self: *ResourceBank, ctx: VulkanContext) void {
...
pub const PreparedResources = struct {
    ...
    pub fn init(renderer: *const VulkanPipeline, allocator: std.mem.Allocator) !PreparedResources {
        var self = PreparedResources{ .allocator = allocator, .ctx = renderer.ctx, .desc_set_layout = renderer.desc_set_layout };
    ...
    fn initDescriptorSet(self: *PreparedResources) !void {
        try self.initDescriptorSetInto(&self.desc_pool, &self.desc_set);
    }

    fn initDescriptorSetInto(self: *PreparedResources, desc_pool: *vk.VkDescriptorPool, desc_set: *vk.VkDescriptorSet) !void {
    ...
    fn bankForId(self: *const PreparedResources, bank_id: u32) ?ResourceBank {
...
pub const VulkanPipeline = struct {
    ...
    pub fn uploadPreparedAtlases(self: *VulkanPipeline, prepared: *PreparedResources, atlases: []const *const CurveAtlas, out_views: anytype) !void {
        try self.uploadPreparedAtlasesWithOptionalCapacityModes(prepared, prepared.allocator, atlases, null, out_views);
    }
    ...
    pub fn uploadPreparedAtlasesWithCapacityModes(
        ...
        try self.uploadPreparedAtlasesAndLayerInfoWithOptionalCapacityModes(prepared, prepared.allocator, atlases, capacity_modes, out_views, layer_infos[0..], layer_info_views[0..]);
    }
    ...
    pub fn uploadPreparedAtlasesAndLayerInfoWithCapacityModes(
        ...
        try self.uploadPreparedAtlasesAndLayerInfoWithOptionalCapacityModes(prepared, scratch, atlases, capacity_modes, out_views, layer_infos, out_layer_info_views);
    }
    ...
    fn uploadPreparedAtlasesWithOptionalCapacityModes(
        ...
        var layer_infos: [0]EmptyLayerInfoUpload = .{};
        var layer_info_views: [0]EmptyLayerInfoView = .{};
        try self.uploadPreparedAtlasesAndLayerInfoWithOptionalCapacityModes(prepared, scratch, atlases, capacity_modes, out_views, layer_infos[0..], layer_info_views[0..]);
    }
    ...
    fn uploadPreparedAtlasesAndLayerInfoWithOptionalCapacityModes(
        ...
        if (!can_incremental and can_overflow_bank) {
            if (!try self.appendTexturePagesIntoNewBank(prepared, scratch, atlases)) {
            ...
            try self.rebuildTextureArrays(prepared, scratch, atlases, capacity_modes, out_views, layer_infos, out_layer_info_views);
        } else if (!try self.appendTexturePages(prepared, scratch, atlases)) {
            try self.rebuildTextureArrays(prepared, scratch, atlases, capacity_modes, out_views, layer_infos, out_layer_info_views);
        } else {
            prepared.fillAtlasViews(atlases, out_views);
            prepared.fillLayerInfoViews(prepared.atlasLayerInfoRows(atlases), layer_infos, out_layer_info_views);
    ...
    pub fn uploadPreparedImages(self: *VulkanPipeline, prepared: *PreparedResources, scratch: std.mem.Allocator, images: []const *const snail_mod.Image, out_views: anytype) !void {
        ...
        try self.ensureImagesRegistered(prepared, scratch, images);
    ...
    fn rebuildTextureArrays(
        ...
        prepared.curve_image = try self.createImage2DArray(curve_w, prepared.allocated_curve_height, prepared.allocated_layer_count, vk.VK_FORMAT_R16G16B16A16_SFLOAT);
        prepared.curve_memory = try self.allocateImageMemory(prepared.curve_image);
        _ = vk.vkBindImageMemory(self.ctx.device, prepared.curve_image, prepared.curve_memory, 0);

        prepared.band_image = try self.createImage2DArray(band_w, prepared.allocated_band_height, prepared.allocated_layer_count, vk.VK_FORMAT_R16G16_UINT);
        prepared.band_memory = try self.allocateImageMemory(prepared.band_image);
        _ = vk.vkBindImageMemory(self.ctx.device, prepared.band_image, prepared.band_memory, 0);

        try self.uploadTextureData(prepared, scratch, atlases, null, prepared.allocated_layer_count, vk.VK_IMAGE_LAYOUT_UNDEFINED);

        prepared.curve_view = try self.createImageView(prepared.curve_image, vk.VK_FORMAT_R16G16B16A16_SFLOAT, prepared.allocated_layer_count);
    ...
    fn appendTexturePages(self: *VulkanPipeline, prepared: *PreparedResources, scratch: std.mem.Allocator, atlases: []const *const CurveAtlas) !bool {
    ...
    fn createAtlasTextureBank(
        ...
        bank.curve_image = try self.createImage2DArray(first_page.curve_width, curve_height, layer_count, vk.VK_FORMAT_R16G16B16A16_SFLOAT);
        bank.curve_memory = try self.allocateImageMemory(bank.curve_image);
        _ = vk.vkBindImageMemory(self.ctx.device, bank.curve_image, bank.curve_memory, 0);

        bank.band_image = try self.createImage2DArray(first_page.band_width, band_height, layer_count, vk.VK_FORMAT_R16G16_UINT);
        bank.band_memory = try self.allocateImageMemory(bank.band_image);
        _ = vk.vkBindImageMemory(self.ctx.device, bank.band_image, bank.band_memory, 0);

        return bank;
    }

    fn appendTexturePagesIntoNewBank(self: *VulkanPipeline, prepared: *PreparedResources, scratch: std.mem.Allocator, atlases: []const *const CurveAtlas) !bool {
        ...
        var bank = try self.createAtlasTextureBank(
            prepared,
            first,
            layer_count,
        ...
        try self.uploadPageDataToImages(
            prepared,
            scratch,
            uploads,
            bank.curve_image,
        ...
        bank.curve_view = try self.createImageView(bank.curve_image, vk.VK_FORMAT_R16G16B16A16_SFLOAT, bank.allocated_layer_count);
        bank.band_view = try self.createImageView(bank.band_image, vk.VK_FORMAT_R16G16_UINT, bank.allocated_layer_count);
        self.updateDescriptorSetViews(bank.desc_set, bank.curve_view, bank.band_view, null, null);
    ...
    fn ensureAtlasImagesRegistered(self: *VulkanPipeline, prepared: *PreparedResources, scratch: std.mem.Allocator, atlases: []const *const CurveAtlas) !void {
        ...
        try self.ensureImagesRegistered(prepared, scratch, images.items);
    }

    fn ensureLayerInfoImagesRegistered(self: *VulkanPipeline, prepared: *PreparedResources, scratch: std.mem.Allocator, layer_infos: anytype) !void {
        var images = std.ArrayListUnmanaged(*const snail_mod.Image).empty;
        defer images.deinit(scratch);
    ...
    fn ensureImagesRegistered(self: *VulkanPipeline, prepared: *PreparedResources, scratch: std.mem.Allocator, images: []const *const snail_mod.Image) !void {
        ...
        if (needs_rebuild) {
            ...
            try self.rebuildImageArray(prepared, scratch);
            return;
        }
        ...
        try self.uploadImagesToArray(prepared, scratch, new_images.items, @intCast(prepared.image_slot_count), vk.VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        prepared.image_slot_count += new_images.items.len;
    }

    fn rebuildLayerInfoTexture(self: *VulkanPipeline, prepared: *PreparedResources, scratch: std.mem.Allocator, atlases: []const *const CurveAtlas, layer_infos: anytype, layer_info_views: anytype) !void {
        ...
        prepared.layer_image = try self.createImage2D(width, total_rows, vk.VK_FORMAT_R32G32B32A32_SFLOAT);
        prepared.layer_memory = try self.allocateImageMemory(prepared.layer_image);
        _ = vk.vkBindImageMemory(self.ctx.device, prepared.layer_image, prepared.layer_memory, 0);
        try self.uploadLayerInfoData(prepared, data, width, total_rows);
        prepared.layer_view = try self.createImageView2D(prepared.layer_image, vk.VK_FORMAT_R32G32B32A32_SFLOAT);
    }

    fn updateDescriptorSet(self: *VulkanPipeline, prepared: *PreparedResources) void {
        self.updateDescriptorSetViews(prepared.desc_set, prepared.curve_view, prepared.band_view, prepared.layer_view, prepared.image_view);
    }
    ...
    fn rebuildImageArray(self: *VulkanPipeline, prepared: *PreparedResources, scratch: std.mem.Allocator) !void {
        ...
        prepared.image_memory = try self.allocateImageMemory(prepared.image_image);
        _ = vk.vkBindImageMemory(self.ctx.device, prepared.image_image, prepared.image_memory, 0);
        try self.uploadImagesToArray(prepared, scratch, all_images[0..image_count], 0, vk.VK_IMAGE_LAYOUT_UNDEFINED);
        prepared.image_view = try self.createImageView(prepared.image_image, vk.VK_FORMAT_R8G8B8A8_SRGB, prepared.allocated_image_count);
    }

    fn uploadImagesToArray(self: *VulkanPipeline, prepared: *PreparedResources, scratch: std.mem.Allocator, images: []const *const snail_mod.Image, start_layer: u32, old_layout: vk.VkImageLayout) !void {
