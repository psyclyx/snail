...
void main()
{
    do
    {
        if (pc.mode == 0)
        {
            highp vec4 sampled = texture(SPIRV_Cross_Combinedu_dst_texu_dst_sampler, snail_io0);
            highp vec4 _194;
            do
            {
                highp float _198 = sampled.w;
                if (_198 <= 0.0)
                {
                    _194 = vec4(0.0);
                    break;
                }
                highp vec3 _205 = clamp(sampled.xyz * (1.0 / _198), vec3(0.0), vec3(1.0));
                highp float _214 = _205.x;
                highp float _221;
        ...
        highp vec4 sampled_1 = texture(SPIRV_Cross_Combinedu_linear_texu_linear_sampler, snail_io0);
        highp vec4 _261;
        do
        {
            highp float _265 = sampled_1.w;
            if (_265 <= 0.0)
            {
                _261 = vec4(0.0);
                break;
            }
            highp vec3 _271 = sampled_1.xyz * (1.0 / _265);
            highp float _280 = max(_271.x, 0.0);
            highp float _289;
            ...
            highp float _283 = max(_271.y, 0.0);
            highp float _301;
            ...
            highp float _286 = max(_271.z, 0.0);
            highp float _313;
