...
{
    highp vec4 paint;
    highp vec3 texcoord_layer;
    ivec2 info;
    uvec4 policy0;
    uvec3 policy1;
    highp vec4 x_targets[4];
    highp vec4 y_targets[4];
    uvec4 x_sources;
    uvec4 y_sources;
};
...
struct SnailAutohintPolicy
{
    ...
    highp float fadeFull;
    highp float xRatio;
    highp float xMaxPx;
    highp float yRatio;
    highp float yMaxPx;
    highp float overshootMinPx;
};
...
highp float snailWarpF(ivec2 _305, int _306, int _307)
{
    int f = _306 + _307;
    highp vec4 _326 = texelFetch(SPIRV_Cross_Combinedu_layer_texSPIRV_Cross_DummySampler, ivec3(snailAhLayerLoc(_305, f >> 2), 0).xy, 0);
    int c = f & 3;
    highp float _309;
    if (c == 0)
    {
...
bool snailAhFinite(highp float v)
{
    bool _375;
    if (!isnan(v))
    {
...
bool snailAhCount(int max_knots, highp float encoded, out int count)
{
    bool _358;
...
highp float snailAhSnap(highp float v, highp float scale)
{
    return round(v * scale) / scale;
}

highp float snailAhStandardWidth(highp float raw, highp float standard, highp float ratio)
{
    bool _2447;
    if (standard > 0.0)
    {
...
bool snailFitAutohintAxis(ivec2 _972, int _973, int _974, int _975, highp float _976, highp float _977, highp float _978, SnailAutohintPolicy _979, inout int _980, inout highp float _981[32], inout highp float _982[32], inout int _983[32])
{
    _980 = 0;
    int i = 0;
    for (;;)
    ...
    for (;;)
    {
        ...
        do
        {
            ...
            if (lowerBlue)
            {
                ...
                for (;;)
                {
                    ...
                    do
                    {
                        ...
                        highp float _2149 = abs(pos[k] - pos[i]);
                        if (_2149 >= nearest)
                        {
                            break;
                        }
            ...
            if (validBlue)
            {
                highp float _2278 = snailWarpF(_972, 12, 2 * blue[i]);
                highp float _2282 = snailWarpF(_972, 12, (2 * blue[i]) + 1);
                if (rounded[i])
                {
                    _1017 = _1747;
                }
    ...
    validBlue = false;
    highp float anchorTarget = 0.0;
    highp float anchorBase = 0.0;
    highp float clusterTarget = 0.0;
    highp float clusterBase = 0.0;
    highp float clusterDesiredRight = 0.0;
    k = 0;
    i = 0;
    encodedCompanion = 0;
    int clusterStems;
    highp float widthUnits;
    highp float clusterTarget_1;
    highp float clusterBase_1;
    highp float clusterTarget_2;
    highp float clusterBase_2;
    highp float clusterDesiredRight_1;
    for (;;)
    {
        ...
        do
        {
            ...
            highp float nominal = snailAhStandardWidth(width[i], _976, spacing);
            if (companionDir == 2)
            {
                _1008 = true;
            }
            ...
            if (partnerAbove)
            {
                ...
                {
                    highp float _2529 = snailAhSnap(pos[i], _978);
                    targets[i] = _2529;
                ...
                highp float _2548 = clusterBase_1;
                highp float _2553 = clusterTarget_1;
                highp float _2559 = clusterTarget_1;
                highp float _2560 = clusterBase_1;
    ...
    if (_986)
    {
        highp float _2701 = clusterDesiredRight - targets[k];
        i = 0;
        for (;;)
        {
    ...
    if (_986)
    {
        highp float span = _979.fadeFull - _979.fadeStart;
        if (span <= 0.0)
        {
            _986 = true;
        }
...
highp float snailInverseWarpAxis(int count, highp float bases[32], highp float targets[32], highp float hinted, out highp float invSlope)
{
    invSlope = 1.0;
    if (count == 0)
    {
    ...
    int _3448 = lo + 1;
    highp float dt = targets[_3448] - targets[lo];
    highp float _3373;
...
highp float snailAhFastTarget(highp vec4 values[4], int idx)
{
    return values[idx >> 2][idx & 3];
}

highp float snailAhFastBase(ivec2 _3577, int _3578, highp float _3579, uvec4 _3580, int _3581)
{
...
highp float snailInverseFastAxis(ivec2 _3521, int _3522, highp vec4 _3523[4], uvec4 _3524, int _3525, highp float _3526, highp float _3527, out highp float _3528)
{
    _3528 = 1.0;
    if (_3522 == 0)
    {
    ...
    highp float _3562 = snailAhFastTarget(_3523, 0);
    if (_3527 <= _3562)
    {
        return (snailAhFastBase(_3521, _3525, _3526, _3524, 0) + _3527) - _3562;
    }
    int _3607 = _3522 - 1;
    highp float _3608 = snailAhFastTarget(_3523, _3607);
    if (_3527 >= _3608)
    {
    ...
    highp float _3642 = snailAhFastTarget(_3523, lo);
    int _3644 = lo + 1;
    highp float _3647 = snailAhFastBase(_3521, _3525, _3526, _3524, lo);
    highp float dt = snailAhFastTarget(_3523, _3644) - _3642;
    highp float _3533;
    if (abs(dt) > 9.9999999747524270787835121154785e-07)
    {
...
CoverageBandSpan computeCoverageBandSpan(highp float coord, highp float eppAxis, highp float bandScale, highp float bandOffset, int bandMax)
{
    ...
    highp float _3789 = max(abs(eppAxis * bandScale) * 0.5, 9.9999997473787516355514526367188e-06);
...
highp vec2 solveHorizPoly(highp vec4 p12, highp vec2 p3)
{
    ...
    highp float _4089 = a.y;
    highp float t1;
    highp float t2;
    if (abs(_4089) < 1.52587890625e-05)
    {
        highp float _4093 = b.y;
    ...
    {
        highp float _4107 = b.y;
        highp float _4110 = _4089 * p12.y;
        highp float sq = snapNearTangentSqrt((_4107 * _4107) - _4110, _4107, _4110);
        if (_4107 >= 0.0)
        {
            highp float q = _4107 + sq;
        ...
        else
        {
            ...
            highp float _4160 = t1;
            t1 = q_1 / _4089;
            t2 = _4160;
        }
    }
    highp float _4165 = a.x;
    highp float _4169 = b.x * 2.0;
    return vec2((((_4165 * t1) - _4169) * t1) + p12.x, (((_4165 * t2) - _4169) * t2) + p12.x);
}

bool accumulateHorizContribution(inout highp float _3937, inout highp float _3938, highp vec2 _3939, highp vec2 _3940, ivec2 _3941, int _3942)
{
    highp vec4 _3959 = texelFetch(SPIRV_Cross_Combinedu_curve_texSPIRV_Cross_DummySampler, ivec4(_3941, _3942, 0).xyz, 0);
    highp vec4 _3986 = texelFetch(SPIRV_Cross_Combinedu_curve_texSPIRV_Cross_DummySampler, ivec4(offsetCurveLoc(_3941, 1), _3942, 0).xyz, 0);
    highp vec4 p12 = vec4(_3959.xy, _3959.zw) - vec4(_3939, _3939);
    highp vec2 p3 = _3986.xy - _3939;
    ...
    if (code != 0u)
    {
        highp vec2 r = solveHorizPoly(p12, p3) * _3940.x;
        if ((code & 1u) != 0u)
        {
            highp float _4187 = r.x;
            _3937 += clamp(_4187 + 0.5, 0.0, 1.0);
            _3938 = max(_3938, clamp(1.0 - (abs(_4187) * 2.0), 0.0, 1.0));
        }
        if (code > 1u)
        {
            highp float _4203 = r.y;
...
highp vec2 solveVertPoly(highp vec4 p12, highp vec2 p3)
{
    highp vec2 a = (p12.xy - (p12.zw * 2.0)) + p3;
    highp vec2 b = p12.xy - p12.zw;
    highp float _4366 = a.x;
    highp float t1;
    highp float t2;
    if (abs(_4366) < 1.52587890625e-05)
    {
        highp float _4370 = b.x;
    ...
    {
        highp float _4384 = b.x;
        highp float _4387 = _4366 * p12.x;
        highp float sq = snapNearTangentSqrt((_4384 * _4384) - _4387, _4384, _4387);
        if (_4384 >= 0.0)
        {
            highp float q = _4384 + sq;
            if (abs(q) < 1.52587890625e-05)
            {
                t1 = 0.0;
            }
        ...
        else
        {
            ...
            highp float _4414 = t1;
            t1 = q_1 / _4366;
            t2 = _4414;
        }
    }
    highp float _4419 = a.y;
    highp float _4423 = b.y * 2.0;
    return vec2((((_4419 * t1) - _4423) * t1) + p12.y, (((_4419 * t2) - _4423) * t2) + p12.y);
}

bool accumulateVertContribution(inout highp float _4289, inout highp float _4290, highp vec2 _4291, highp vec2 _4292, ivec2 _4293, int _4294)
{
    highp vec4 _4308 = texelFetch(SPIRV_Cross_Combinedu_curve_texSPIRV_Cross_DummySampler, ivec4(_4293, _4294, 0).xyz, 0);
    highp vec4 _4314 = texelFetch(SPIRV_Cross_Combinedu_curve_texSPIRV_Cross_DummySampler, ivec4(offsetCurveLoc(_4293, 1), _4294, 0).xyz, 0);
    highp vec4 p12 = vec4(_4308.xy, _4308.zw) - vec4(_4291, _4291);
    highp vec2 p3 = _4314.xy - _4291;
    ...
    if (code != 0u)
    {
        highp vec2 r = solveVertPoly(p12, p3) * _4292.y;
        if ((code & 1u) != 0u)
        {
            highp float _4441 = r.x;
            _4289 -= clamp(_4441 + 0.5, 0.0, 1.0);
            _4290 = max(_4290, clamp(1.0 - (abs(_4441) * 2.0), 0.0, 1.0));
        }
        if (code > 1u)
        {
            highp float _4457 = r.y;
...
{
    highp float _4536 = clamp(cov, 0.0, 1.0);
    highp float _4537 = max(coverage_exponent, 1.52587890625e-05);
    highp float _4532;
...
highp float evalGlyphCoverage(highp vec2 _3699, highp vec2 _3700, highp vec2 _3701, ivec2 _3702, ivec2 _3703, highp vec4 _3704, int _3705, highp float _3706)
{
...
highp vec4 snailAutohintFragment(AutohintVaryings _120, int _121, int _122, highp float _123, int _124)
{
    highp vec4 _233 = texelFetch(SPIRV_Cross_Combinedu_layer_texSPIRV_Cross_DummySampler, ivec3(_120.info, 0).xy, 0);
    highp vec4 _264 = texelFetch(SPIRV_Cross_Combinedu_layer_texSPIRV_Cross_DummySampler, ivec3(snailAhLayerLoc(_120.info, 1), 0).xy, 0);
    int _274 = floatBitsToInt(_233.z);
    highp vec2 rc = _120.texcoord_layer.xy;
    highp vec2 epp = _fwidth(_120.texcoord_layer.xy);
    int blueCount = 0;
    int featureXCount = 0;
    int featureYCount = 0;
    ...
    int yCount = _136;
    highp float slopeX = 1.0;
    highp float slopeY = 1.0;
    int _536 = xCount;
    ...
    if (valid)
    {
        highp float _552 = snailWarpF(_120.info, 0, 8);
        highp float _553 = snailWarpF(_120.info, 0, 9);
        ...
        if (_144)
        {
            ...
            highp float _154[32] = bases;
            highp float _155[32] = targets;
            highp float _3361 = snailInverseWarpAxis(xCount, _154, _155, rc.x, slopeX);
            rc.x = _3361;
        }
        if (valid)
        {
        ...
        if (valid)
        {
            ...
            highp float bases_1[32];
            highp float targets_1[32];
            int sources_1[32];
            ...
            highp float _160[32] = bases_1;
            highp float _161[32] = targets_1;
            highp float _3502 = snailInverseWarpAxis(yCount, _160, _161, rc.y, slopeY);
            rc.y = _3502;
        }
    }
    if (!fallbackX)
    {
        highp vec4 _162[4] = _120.x_targets;
        highp float _3518 = snailInverseFastAxis(_120.info, xCount, _162, _120.x_sources, xRun, snailWarpF(_120.info, 0, 11), rc.x, slopeX);
        rc.x = _3518;
    }
    if (!fallbackY)
    {
        highp vec4 _163[4] = _120.y_targets;
        highp float _3679 = snailInverseFastAxis(_120.info, yCount, _163, _120.y_sources, yRun, 0.0, rc.y, slopeY);
        rc.y = _3679;
    }
    highp vec2 epp_1 = epp * vec2(slopeX, slopeY);
    highp float _3696 = evalGlyphCoverage(rc, epp_1, vec2(1.0 / max(epp_1.x, 1.52587890625e-05), 1.0 / max(epp_1.y, 1.52587890625e-05)), ivec2(int(_233.x + 0.5), int(_233.y + 0.5)), ivec2((_274 >> 16) & 65535, _274 & 65535), _264, _121 + int(_120.texcoord_layer.z), _123);
    if (_3696 < 0.0039215688593685626983642578125)
    {
        discard;
    }
    highp vec4 premul = premultiplyColor(_120.paint, _3696);
    highp vec4 _164;
    if (_124 != 0)
    {
        _164 = vec4(premul.w);
...
void main()
{
    ...
    highp vec4 _117 = snailAutohintFragment(_18, pc.layer_base, pc.output_srgb, pc.coverage_exponent, pc.mask_output);
