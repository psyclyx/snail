...
pub fn translateBBox(bbox: BBox, delta: Vec2) BBox {
    return .{
...
pub fn bboxCenter(bbox: BBox) Vec2 {
    return .{
...
pub fn translatePaint(paint: Paint, delta: Vec2) Paint {
    return paint_mod.mapToLocal(paint, Transform2D.translate(-delta.x, -delta.y)).?;
}

pub fn fillStyleForStroke(style: StrokeStyle) FillStyle {
    return .{
        .paint = style.paint,
    };
}
...
pub fn buildCircularSectorPath(
    path: *Path,
    center: Vec2,
    outer_radius: f32,
    inner_radius: f32,
