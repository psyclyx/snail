...
pub fn shapedRunPicture(
    ...
    const shapes = try allocator.alloc(Shape, shaped.glyphs.len);
    ...
    // Picture.from copies shapes into a fresh allocation, so the scratch
    // we built above is freed by the defer.
    return Picture.from(allocator, shapes);
}
